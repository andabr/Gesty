...
public class MyGestureOverlay extends FrameLayout {
    ...
    public MyGestureOverlay(Context context, AttributeSet attrs, int defStyle) {
        ...
        mFadeDuration = 1000;// a.getInt(R.styleable.GestureOverlayView_fadeDuration, (int) mFadeDuration);
        mFadeOffset = 420;//a.getInt(R.styleable.GestureOverlayView_fadeOffset, (int) mFadeOffset);
    ...
    private void touchDown(MotionEvent event) {
        ...
        mTotalLength = 0;
        mIsGesturing = false;
        //TODO make sure to cancel animation quickly!!!
        if (mEffectPath != null) {
            mEffectPath.reset();
            mPhase = 0;
    ...
    private class FadeOutRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mIsFadingOut) {
                if (!hasFired) {
                	fireGesturePerformedSynchronously();
                	hasFired = true;
                }
                ...
                if (duration > mFadeDuration) {
                    fireGesturePerformedSynchronously2();
                    if (mEffectPath != null) {
                        mEffectPath.reset();
                        mPhase = 0;
        				getGesturePaint().setPathEffect(null);
	...
	private void fireGesturePerformedSynchronously() {
		if (mSyncListener != null) {
			mSyncListener.onGesturePerformed(this, mCurrentGesture);
		}
	}
	
	private void fireGesturePerformedSynchronously2() {
		if (mSyncListener != null) {
			mSyncListener.onGesturePerformed2(this, mCurrentGesture);
		}
	}
