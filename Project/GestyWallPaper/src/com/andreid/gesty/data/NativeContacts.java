...
public class NativeContacts {	
	...
	public interface Projections {
		...
		/** Projection for retrieving display names */
		final String[] DISPLAY_NAME = new String[] {
	    ...
	    try {
	        ...
	        if (cursor != null) {
	            ...
	            while (cursor.moveToNext()) {
	                ...
	                contact.addEmail(new ContactDetail(ContactDetail.TYPE_EMAIL, type, label, value, isPrimary, detailId));
	            }
	        }
	    } catch(Exception exc) {
	        exc.printStackTrace();
	...
	public static String getEmailByDetailId(ContentResolver resolver, long detailId) {
	    if (detailId == 0) {
	        return null;
	    }
	    Cursor cursor = null;
	    try {
	        cursor = resolver.query(Email.CONTENT_URI,
	                ...
	                Email._ID + "=?" + " AND " + Data.MIMETYPE + "='" + Email.CONTENT_ITEM_TYPE + "'",
	                new String[] {String.valueOf(detailId)}, null);
	        if (cursor != null) {
	...
	public static void readContactPhoneNumbers(final ContentResolver resolver,
		...
		try {
		    ...
		    while (cursor.moveToNext()) {
                ...
                contact.addPhoneNumber(new ContactDetail(ContactDetail.TYPE_PHONE, type, label, number, isPrimary, id));
            }
		} catch(Exception exc) {
		    exc.printStackTrace();
		} finally {
	...
	public static long getContactPhotoId(ContentResolver resolver, long contactId) {
		Uri contactUri = ContentUris.withAppendedId(ContactsContract.Contacts.CONTENT_URI, contactId);
		Cursor cursor = resolver.query(contactUri, new String[] {Contacts.PHOTO_ID}, null, null, null);
		try {
