...
public class PreferencesCache {
	...
	//Default values
	private static final int GESTURE_THUMBNAIL_SIZE_DEFAULT = 88;
	
	private static final int GESTURE_THUMBNAIL_INSET_DEFAULT = 8;
	
	private static final int GESTURE_PREDICTION_ACCURACY_DEFAULT = 2;
    ...
    private static void setFloat(Context context, String key, float value) {
        ...
        if (!editor.commit()) {
            throw new NullPointerException(TAG+".setFloatValue() Failed to set key[" + key
                    + "] with value[" + value + "]");
        }
    }
    ...
    public static int getGestureSize(Context context) {
    	return getInt(context, GESTURE_THUMBNAIL_SIZE, GESTURE_THUMBNAIL_SIZE_DEFAULT);
    }
    ...
    public static int getGestureInset(Context context) {
    	return getInt(context, GESTURE_THUMBNAIL_INSET, GESTURE_THUMBNAIL_INSET_DEFAULT);
    }
    ...
    public static float getGestureAccuracy(Context context) {
    	return getFloat(context, GESTURE_PREDICTION_ACCURACY, GESTURE_PREDICTION_ACCURACY_DEFAULT);
    }
}
